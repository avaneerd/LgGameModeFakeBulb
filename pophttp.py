...
def server_loop(address, handler):
    ...
    while True:
        ...
        if not config.ip_allowed(address[0]):
            log.warning('recv filtering packet %s', data, extra=dict(clientip=address[0], clientport=address[1]))
            continue
        packet = lifx.Message.decode(data)
        if packet is None:
            log.warning('recv Unknown packet type %s', data, extra=dict(clientip=address[0], clientport=address[1]))
            continue

        log.warning('recv %r', packet, extra=dict(clientip=address[0], clientport=address[1]))
        ...
        if packet.code == lifx.Message.Device_GetVersion.code:
            ...
            resp = lifx.Message.Light_State(hue=0, saturation=655, brightness=65535, kelvin=2500, dim=0, power=powerState, label='LG set Game mode', tags=0)
        ...
        if resp is not None:
            log.warning('send %s', str(resp), extra=dict(clientip=address[0], clientport=address[1]))
